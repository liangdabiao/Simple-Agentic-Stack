...
**【必须】参考图片**：当用户提供了产品照片/参考图时，**必须**使用 `--image` 参数传入。这是提升产品外观一致性的**最关键**手段，忽略参考图会导致生成结果与真实产品差异巨大。
...
9. **【必须】如果用户要求直接出图，必须调用 `scripts/generate_image.py`**；如果用户提供了参考产品图，**必须**传入 `--image` 参数。
...
- 主体：产品、人物、场景、物体或抽象概念。
- **【关键】参考图**：用户是否提供了参考产品图？如果有，生成时**必须**使用 `--image` 参数。
- 受众和使用语境。
...
- **【关键】如果用户提供了参考产品图，传入 `--image` 参数比文字描述更有效。
...
- **`--image`【必须】**：参考产品图片路径，**当用户提供参考图时必须使用此参数**
